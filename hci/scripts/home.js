const alarmTemplate = document.getElementById("alarm-template").firstElementChild;
const alarmList = document.getElementById("alarm-list");

if(localStorage.alarms == undefined) {
    alarms = [
        {"name": "Wake up", "time": "08:00:00", "timezone": "CET", "enabled": true},
        {"name": "Join meeting A", "time": "12:30:00", "timezone": "UTC", "enabled": true},
        {"name": "Join meeting B", "time": "13:00:00", "timezone": "UTC", "enabled": false}
    ];
...
function addAlarm(name, time, timezone, id) {
    ...
    alarm.querySelector("#settings-button").setAttribute("href", "edit_alarm.html?id=" + id)
...
function updateAlarmButton(id, enabled) {
    ...
    if(enabled) {
        alarm.querySelector(".enable-disable").innerHTML = "<i class=\"fas fa-toggle-on\"></i>";
        alarm.querySelector(".enable-disable").style.color = "#348745"
    } else {
        alarm.querySelector(".enable-disable").innerHTML = "<i class=\"fas fa-toggle-on fa-rotate-180\"></i>";
